...
const callback = async (msg: Message, args: string[]) => {
	...
	if (!emotes || !emotes.length) return msg.channel.send('You did not provide any valid emotes!');
	...
	else return m.edit(`${msg.client.constants.emojis.success} Successfully yoinked ${created.length} emotes: ${created.join(' ')}`);
};
