...
export default async (client: Client, oldMember: GuildMember, newMember: GuildMember) => {
	if (oldMember.partial as boolean) return;
	...
	if (oldMember.roles.cache.has(boosterRole.id) && !newMember.roles.cache.has(boosterRole.id)) {
		void client.getChannel('info').send(`${newMember.toString()} (${newMember.user.tag}) just unboosted ${client.constants.emojis.stinks}`);
	...
	// POGGERS NEW BOOSTER
		...
		void client.getChannel('info').send(`${newMember.toString()} (${newMember.user.tag}) just boosted ${client.constants.emojis.stonks}`);
		...
		const text = dbEntry
			? `Hello ${newMember.toString()}, thank you for re-boosting! I restored your colour role :)`
			: `Hello ${newMember.toString()}, thank you for boosting! Make sure to pick up the beta apk in <#${
					client.getChannel('testers').id
		...
		setTimeout(() => void client.getChannel('boosters').send(text), 1000 * 5);
	}
};
