...
module.exports = {
    ...
    execute(message, args) {
        ...
        if(args[0] == '000000') return message.channel.send('For black, use hex 111111.')
        
        var name = ''
        if(args[1]) for(let i = 1; i < args.length; i++) name += args[i].replace(/\W/g, '') + ' '; else name = args[0].toUpperCase() + ' '
        name = name.slice(0, -1)
        if(name.toLowerCase().includes('admin') || name.toLowerCase().includes('mod') || name.toLowerCase().includes('janitor')) return message.channel.send('Not funny, didn\'t laugh. Please don\'t impersonate staff, kthxbai.')
