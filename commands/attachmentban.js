...
module.exports = {
	...
	execute(message, args) {
        ...
        if(!member) return message.channel.send('Please mention a member to attachment-ban.')
        var reason = []
        ...
        member.addRole(message.guild.roles.find(role => role.name === "attachment-ban")).then((member) => {
            message.channel.send(success);
            message.client.channels.get(logchannel).send(success)
        }).catch(() => {
            message.channel.send('Oops, something went wrong. Please make sure you got a role called `attachment-ban` set up on this server which is lower than my highest role.');
        });
